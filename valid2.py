...
def UNET(input_shape):
    ...
    x, skip4 = downsample_block(x, 4, 64)
    x, _ = downsample_block(x, 5, 128, pooling_on=False)
    # upsampling
    x = upsample_block(x, skip4, 6, 64)
    x = upsample_block(x, skip3, 7, 32)
    x = upsample_block(x, skip2, 8, 16)
    x = upsample_block(x, skip1, 9, 8)
